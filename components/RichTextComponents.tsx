...
export const RichTextComponents = {
    ...
    image: ({ value }: any) => {
      return (
        <div className='relative w-full h-96 m-10 mx-auto'>
          <Image className='object-contain' fill src={urlFor(value).url()} alt='Blog Post Image' />
        </div>
      );
    },
  },
  ...
  block: {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className='border-l-primaryGreen border-l-4 pl-5 py-5 my-5'>
        {children}
      </blockquote>
    ),
  },
    ...
    link: ({ children, value }: any) => {
      ...
      return (
        <Link
          href={value.href}
          rel={rel}
          className='underline decoration-primaryGreen hover:decoration-black'>
          {children}
        </Link>
      );
    },
